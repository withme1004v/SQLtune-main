...
2.계정생성하기 
##########################################################################
-- mysql 은 기본적으로 계정생성시 원격으로 접근 가능 여부를 지정해야 한다. 
-- '계정명'@'localhost'   : 서버와 클라이언트가 동일한 공간에 있어야 한다. 
--  workbench는 직접 접근이 안되지만 putty를 통해서 서버접근해서 디비서버에 접근하거나(콘솔만)
--  호스팅 업체 : phpmyadmin를 설치한다. mysql서버를 접근할 수 있는 웹서버를 만들어서 접근가능하게 
-- 'localhost'대신에 아이피나, 도메인 , '%'  
...
root 계정도 기본적으로는 로컬만 접근 가능 권한을 별도로 부여해야 한다. 
...
수정후 mysql 재시작 : 우분트에서 sudo -관리자의 권한으로 
...
#####################
백업하기 -- cmd 창에서 한다. 백업 파일을 저장할 폴더로 이동한다 
#####################
전체 백업 
mysqldump -u root -p --all-databases > all_backup_20250607.sql
...
복원은 부등호를 반대로 
디비들어가서 drop database 데이터베이스명;

mysql -u root -p < mydb_backup.sql

자동 백업을 원할경우 배치파일이나 쉘파일을 만들어서 cron이나 작업스케줄러에 등록하기 

########### 백업과 복원 #################
cmd- 백업도 복원도 
백업명령어 : mysqldump -u root -p 백업할디비명 > 백업파일명   
...
복합인덱스는 앞의 조건을 먼저 만족하게 하는게 효율적이다 
...
########## primary key가 있을 경우에 range 스캔을 한다. 
MySQL [tuning]> EXPLAIN SELECT * FROM 사원 WHERE 사원번호 BETWEEN 100001 AND 200000;
+----+-------------+--------+------------+-------+---------------+---------+---------+------+-------+----------+-------------+
| id | select_type | table  | partitions | type  | possible_keys | key     | key_len | ref  | rows  | filtered | Extra       |
+----+-------------+--------+------------+-------+---------------+---------+---------+------+-------+----------+-------------+
|  1 | SIMPLE      | 사원   | NULL       | range | PRIMARY       | PRIMARY | 4       | NULL | 20682 |   100.00 | Using where |
+----+-------------+--------+------------+-------+---------------+---------+---------+------+-------+----------+-------------+
...
alter table 테이블명 add constraint 유니크제약조건명  unique(필드);

alter table 사원 add constraint unique_사원번호2 unique(사원번호2);


############# 제약조건 확인 #########################
SELECT TABLE_NAME, CONSTRAINT_NAME, COLUMN_NAME
FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
WHERE TABLE_SCHEMA = 'tuning' AND TABLE_NAME = '사원';
...
| id | select_type | table  | partitions | type  | possible_keys                        | key                  | key_len | ref   | rows | filtered | Extra |
+----+-------------+--------+------------+-------+--------------------------------------+----------------------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | 사원   | NULL       | const | unique_사원번호2,idx_users_email     | unique_사원번호2     | 5       | const |    1 |   100.00 | NULL  |
+----+-------------+--------+------------+-------+--------------------------------------+----------------------+---------+-------+------+----------+-------+
...
############ 자동형전환으로 인한 인덱스 오류 발생 ######################
#################### unique 제약조건 부여하기 ##################
select * from 사원 limit 20000, 10;
select * from 사원 where 사원번호3='30001';

-- 컬럼추가 (오래걸림)
alter table 사원 add column 사원번호3 varchar(20);
update 사원 set 사원번호3 = 사원번호;
...
show index from 사원;
explain select * from 사원 where 이름='Tzvetan'; -- 인덱스 검색 , ref 
...
select getdept(deptno), empno from emp limit 30;
